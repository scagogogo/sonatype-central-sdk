...
func TestSearchByArtifactId(t *testing.T) {
	...
	// 测试搜索
	artifacts, err := client.SearchByArtifactId(context.Background(), "test-artifact", 3)
	assert.NoError(t, err)
	assert.NotNil(t, artifacts)
	assert.Len(t, artifacts, 3)

	// 检查请求格式
	assert.Contains(t, requestPath, "q=a%3Atest-artifact")
	assert.Contains(t, requestPath, "rows=3")
...
func TestSearchByArtifactIdWithNoLimit(t *testing.T) {
	// 设置模拟服务器，模拟分页
	requestCount := 0
	_, client := setupMockServer(t, func(w http.ResponseWriter, r *http.Request) {
		requestCount++
	...
	// 测试无限制搜索 (会使用迭代器)
	artifacts, err := client.SearchByArtifactId(context.Background(), "popular-artifact", 0)
	assert.NoError(t, err)
	assert.NotNil(t, artifacts)
...
func TestSearchByArtifactIdError(t *testing.T) {
	// 测试错误处理
	_, client := setupMockServer(t, func(w http.ResponseWriter, r *http.Request) {
		mockErrorResponse(w, 500, "Internal Server Error")
	})

	artifacts, err := client.SearchByArtifactId(context.Background(), "error-artifact", 10)
	assert.Error(t, err)
...
func TestIteratorByArtifactId(t *testing.T) {
	...
	// 创建迭代器
	iterator := client.IteratorByArtifactId(context.Background(), "test-artifact")
	...
	assert.Equal(t, 1, pageCount) // 由于空检查，当找到所有结果后不会进行第二次请求
}
